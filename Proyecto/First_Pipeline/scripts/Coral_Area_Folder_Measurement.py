...
fileset = glob.glob(inputfolder)

n_images = len(fileset)

output_final_name = './'+inputfolder+'_areas.dat'
...
for file_input_image in fileset:
	name_with_extension = file_input_image.strip(inputfolder)

	output_name = './measurements_intermediate_steps_'+name_with_extension+'.dat'

	fileout = open(output_name, 'w')

	image = pylab.imread(file_input_image)
...
# Now we get the area of the square
	...
	file_final_out.write("%s %f \n"%(name_with_extension, area_coral_cm_2))
	...
	plt.savefig("final_image"+name_with_extension)
	plt.xlabel("y(pixels)")
	plt.ylabel("x(pixels)")
	plt.close(fig)
